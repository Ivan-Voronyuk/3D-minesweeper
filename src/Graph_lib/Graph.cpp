...
void Polygon::add(Point p) {
  ...
  for (int i = 1; i < np - 1; ++i) {
    Point ignore{0, 0};
    if (line_segment_intersect(point(np - 1), p, point(i - 1), point(i), ignore)) error("intersect in polygon");
  }
...
void Ellipse::draw_lines() const {
  ...
  if (color().visibility()) fl_arc(point(0).x, point(0).y, w + w, h + h, 0, 360);
}
